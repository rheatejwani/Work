...
foreign_lang: categorical variable("Yes" for any song whose genre has a foreign country/language in it)
...
spotify_data <- spotify_data %>%
  mutate(foreign_lang = case_when(
    ...
    | `Top Genre` == "dutch pop" |`Top Genre` == "dutch americana" |
      ...
      `Top Genre` == "latin alternative"| `Top Genre` == "latin jazz" ~ "Yes"
  ))
#HOW TO DO ELSE FOR CASE_WHEN ~ "no"
```
...
```{r visualized_genres}
ggplot(data = spotify_data, mapping = aes(x = broad_genre)) +
  facet_wrap(~decade) +
  geom_bar() +
    labs(x = "Broad Genre", 
